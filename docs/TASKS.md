...
> **依賴順序**：B1→B2→B3；C1→C2→C3（C1 需 B1）；D1→D2、D3→D4→D5（D2/D4 需 D1）。
...
2. `npm run dev` → 填 1991-10-05 14:00 女 → 報告出現 3 張 canvas 圖、
   console 零錯誤。
3. `grep -r "from '../analysis" src/ui/` 零結果（D-015 修復證明）。
...
**驗收條件**：`npm test` 全綠；空殼 Report 的 violations === []；
...
**驗收條件**：`npm test` 全綠；恰好 5 個 scenario、id 固定；
...
**驗收條件**：`npm test` 全綠；bazi periods 數 = daYun 步數、ziwei periods = 12；
...
**驗收條件**：`npm test` 全綠（不新增）；`npm run dev` 手動驗收：
