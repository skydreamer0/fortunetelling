...
2. **修改前先讀架構文件**：依序讀 `docs/ARCHITECTURE.md` →（v2 任務另讀 `docs/ARCHITECTURE-V2.md`）→ `docs/DECISIONS.md` →
...
5. **測試失敗不得進入下一任務**：交付前 `bun test` 必須全綠（既有測試＋你新增的測試）。
...
- 具體描述：<哪個測試/哪條規則/缺什麼>
- 已做的變更：<清單；若已回退寫「已回退」>
- bun test 現況：<N pass / M fail，貼出失敗測試名>
