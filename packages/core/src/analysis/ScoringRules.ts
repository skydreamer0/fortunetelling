...
export interface ScoringRule {
  /** Unique rule identifier (e.g. "bazi_fire_strength") */
  id: string;
  /** Radar type this rule belongs to */
  radarType: string;
  /** Axis this rule scores */
  axisName: string;
  /** Originating engine (e.g. "bazi", "ziwei", "vedic") */
  sourceSystem: string;
  /** Machine-readable formula expression */
  formula: string;
...
/** Snapshot returned by {@link ScoringRules#exportRules} (Report `scoringRules`). */
export interface ScoringRulesExport {
  exportedAt: string;
  totalRules: number;
  byRadarType: Record<string, ScoringRule[]>;
  bySystem: Record<string, ScoringRule[]>;
}
...
export class ScoringRules {
  ...
  #rulesById: Map<string, ScoringRule>;

  #rulesByRadar: Map<string, ScoringRule[]>;

  #rulesBySystem: Map<string, ScoringRule[]>;
   ...
   */
  #validateRule(rule: ScoringRule) {
    const required: (keyof ScoringRule)[] = [
   ...
   */
  getRule(ruleId: string): ScoringRule | null {
   ...
   *
   * @param radarType - e.g. "element_balance", "planetary_dignity"
   */
  getRulesForRadar(radarType: string): ScoringRule[] {
   ...
   *
   * @param sourceSystem - e.g. "bazi", "vedic"
   */
  getRulesForSystem(sourceSystem: string): ScoringRule[] {
   ...
   */
  formatRule(rule: ScoringRule): FormattedRule {
   ...
   */
  formatRuleAsText(rule: ScoringRule): string {
   ...
   */
  exportRules(): ScoringRulesExport {
    const byRadarType: Record<string, ScoringRule[]> = {};
    for (const [type, rules] of this.#rulesByRadar) {
      byRadarType[type] = rules.map(r => ({ ...r }));
    }

    const bySystem: Record<string, ScoringRule[]> = {};
   ...
   */
  getRadarTypeLabel(radarType: string): string {
   ...
   */
  getSourceLabel(sourceSystem: string): string {
