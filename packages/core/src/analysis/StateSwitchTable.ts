 ...
 *
 * 吃 {@link import('./LayerClassifier').LayerClassification}，把 L3 情境部件
...
import type { LayerClassification, ClassifiedComponent } from './LayerClassifier';

// ─── Type Definitions ───────────────────────────────────────────────────────

export type ScenarioId = 'first_meeting' | 'intimate_stable' | 'conflict' | 'low_pressure' | 'workplace';

export interface Cell {
  /** 至少 1 個 */
  sources: { system: string; componentId: string; name: string }[];
  /** 「在…情境下你會傾向…」語氣（過 HonestyGuard L3） */
  expression: string;
  /** 恆為 true：⚠️待驗證假說 */
  hypothesis: true;
}
 ...
 */
export const SCENARIO_DEFINITIONS: ReadonlyArray<{ id: ScenarioId; name: string }> = Object.freeze([
 ...
 */
export const CATEGORY_SCENARIO_MAP: Record<string, ScenarioId[]> = {
 ...
 * 印星=學習與支持、比劫=同儕與競合。
 */
export const TEN_GOD_GROUP_SCENARIO_MAP: Record<string, ScenarioId[]> = {
 ...
 */
export const TEN_GOD_GROUP_EXPRESSIONS: Record<string, Partial<Record<ScenarioId, string>>> = {
  官殺: {
 ...
 */
function starsLabel(facet: { majorStars?: { name: string }[] } | null | undefined): string {
 ...
 */
function lintedL3(expression: string): string {
...
function toSource(component: ClassifiedComponent): { system: string; componentId: string; name: string } {
 ...
 */
function soulVsBodyExpression(component: ClassifiedComponent, scenarioId: ScenarioId, scenarioName: string): string | null {
 ...
 */
function sanFangExpression(component: ClassifiedComponent, scenarioId: ScenarioId, scenarioName: string): string | null {
 ...
 * 沒觀察到的角色群不得編故事，D-008）。
 */
function tenGodsCells(component: ClassifiedComponent, scenarioId: ScenarioId, scenarioName: string): Cell[] {
  const cells: Cell[] = [];
...
export class StateSwitchTable {
   ...
   * @returns 恆為 5 個、順序固定
   */
  static build(classification: LayerClassification): Scenario[] {
