...
const DAY_MASTER_TRAITS = Object.freeze({
...
const STAR_TRAITS = Object.freeze({
  紫微: '統籌與主導感',
  天機: '觀察、策畫與應變力',
  太陽: '公開承擔與照應他人的傾向',
  武曲: '執行、取捨與資源意識',
...
const LIFE_PATH_TRAITS = Object.freeze({
  1: '自主、開創與把想法化成第一步',
  2: '感受、協作與在關係中找到平衡',
  3: '表達、創意與讓氣氛流動起來',
  4: '結構、秩序與把事情穩定完成',
...
/**
 * @typedef {Object} SummarySentence
 * @property {string} id
 * @property {string} text
 * @property {'L0'} layer
 * @property {SummarySource[]} sources
 */

/**
 * @typedef {Object} Summary
 * @property {number} version
 * @property {SummarySentence[]} sentences
 * @property {string[]} sourceSystems
...
function featuredStars(stars) {
  ...
  return stars
    .filter(star => typeof star?.name === 'string' && star.name.length > 0)
    .map((star, index) => ({ ...star, _sourceIndex: index }))
    .sort((a, b) => {
...
function starTraitPhrase(stars) {
...
export class SummaryBuilder {
  /**
   * @param {Array<{ engineId: string, engineName?: string, components?: Object[] }>} systemResults
   * @returns {Summary}
   */
  static build(systemResults) {
