 ...
 */
function note(assets: string[], liabilities: string[]): AxisNote {
 ...
 */
export const AXIS_NOTES: Readonly<Record<string, Readonly<Record<string, AxisNote>>>> = Object.freeze({
  // ── 八字五行 5 軸（佔比高時的兩面性） ─────────────────────────────────
 ...
 *
 * @param radarId  - Radar id (e.g. 'bazi_element_balance')
 * @param axisLabel - Axis display label (e.g. '木', '命宮', '7')
 */
export function getAxisNotes(radarId: string, axisLabel: string): { assets: string[]; liabilities: string[] } {
