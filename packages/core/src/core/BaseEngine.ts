...
import { SystemResult, type Component, type ComponentInput } from './models/SystemResult';
...
export class BaseEngine {
   ...
   *
   * @param birthData - expected to be a {@link BirthData}; anything else yields an error result
   */
  run(birthData: unknown): SystemResult {
    ...
    try {
      ...
      return this.#failure(birthData, `輸入資料無效：${(err as Error).message}`, start);
    }
    ...
    try {
      ...
      return this.#failure(birthData, `計算失敗：${(err as Error).message}`, start);
    }
  }
   ...
   * @abstract
   */
  _compute(_birthData: BirthData): SystemResult | Component[] {
   ...
   */
  result(components: ComponentInput[] = []): SystemResult {
   ...
   */
  #normalize(raw: unknown): SystemResult {
   ...
   */
  #failure(_birthData: unknown, message: string, start: number): SystemResult {
  ...
  /** @returns Rounded elapsed milliseconds. */
