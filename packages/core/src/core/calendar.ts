...
/** Lunar date input (month 1–12; `isLeap` marks a leap month). */
export interface LunarDateInput extends YMD {
  isLeap?: boolean;
}
...
export function solarToLunarDate({ year, month, day }: YMD): Required<LunarDateInput> {
...
export function lunarToSolarDate({ year, month, day, isLeap = false }: LunarDateInput): YMD & { iso: string } {
