 ...
 */
export const BRIGHTNESS_WEIGHTS: Readonly<Record<string, number>> = Object.freeze({
  廟: 7,
  旺: 6,
  得: 5,
  利: 4,
 ...
 */
export function brightnessScore(brightness: string | undefined): number | null {
...
export class ZiweiEngine extends BaseEngine {
  id = 'ziwei';
  name = '紫微斗數';
  /** iztro output language. */
  declare language: Language;
  /** Evaluation date for the time-varying layers; null = "now" at compute time. */
  declare asOf: Date | string | null;

  /**
   * @param options.language - iztro output language (default 'zh-TW'). Must be a
   ...
   */
  constructor({ language = 'zh-TW', asOf = null }: { language?: Language; asOf?: Date | string | null } = {}) {
    super();
  ...
  #addNatal(result: SystemResult, astrolabe: Astrolabe): void {
    result.add({
      id: 'natal_summary',
      name: '命盤總覽',
      category: 'natal',
   ...
   */
  #addPalacesAndStars(result: SystemResult, astrolabe: Astrolabe): void {
   ...
   */
  #addFourTransforms(result: SystemResult, astrolabe: Astrolabe): void {
    for (const palace of astrolabe.palaces) {
   ...
   */
  #addContextFacets(result: SystemResult, astrolabe: Astrolabe): void {
    try {
   ...
   * the engine.
   */
  #addHoroscope(result: SystemResult, astrolabe: Astrolabe, birth: BirthData): void {
   ...
   *
   * @param asOfNominalAge - 虛歲 at asOf, or null if unknown.
   */
  #addDaXianSequence(result: SystemResult, astrolabe: Astrolabe, birth: BirthData, asOfNominalAge: number | null): void {
    ...
    decadalPalaces.forEach((palace, i) => {
      ...
      try {
        ...
        result.warn(`大限 ${index} 四化計算失敗（${probeDate.getFullYear()}）：${(err as Error).message}`);
      }
   ...
   */
  #serializePalaceFacet(palace: Palace): SerializedPalaceFacet {
    return {
      name: palace.name,
      index: palace.index,
      earthlyBranch: palace.earthlyBranch,
   ...
   */
  #serializeStar(star: Star): SerializedStar {
    return {
      name: star.name,
      type: star.type,
      brightness: star.brightness ?? '',
