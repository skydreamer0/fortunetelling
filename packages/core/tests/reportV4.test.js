...
const engine = (report, id) => report.engines.find(e => e.engineId === id);
const comp = (report, engineId, id) => engine(report, engineId).components.find(c => c.id === id)?.value;
const natal = (report) => {
...
test('Taipei 2026-02-15 13:03 → 真太陽時 12:55 → 時柱 壬午（民用時 13:03 為 癸未）', SLOW, () => {
  const input = { year: 2026, month: 2, day: 15, hour: 13, minute: 3, gender: 'male' };
...
test('23:30 出生：紫微用晚子（timeIndex 12），v3 誤用同日早子', SLOW, () => {
  const input = { year: 1990, month: 6, day: 15, hour: 23, minute: 30, gender: 'female' };
  const report = analyze(input, { asOf: AS_OF });
  ...
  const legacy = new ZiweiEngine({ asOf: new Date(AS_OF) }).run(new BirthData(/** @type {any} */ (input)));
...
test('流年以立春分年：asOf 2027-01-15 → { year: 2026, 丙午 }；立春後 → { 2027, 丁未 }', SLOW, () => {
  const input = { year: 1990, month: 6, day: 15, hour: 10, minute: 30, gender: 'female' };
...
test('useTrueSolarTime:false 對非邊界台北出生重現 v3 引擎全部部件（parity）', SLOW, () => {
  for (const input of [
    ...
    const v3 = createDefaultRegistry({ asOf: new Date(AS_OF) }).runAll(new BirthData(/** @type {any} */ (input)));
...
test('出生地解析：birthplace > cityId > 舊經緯度（Asia/Taipei）> 預設台北', SLOW, () => {
  const base = { year: 1991, month: 10, day: 5, hour: 14, minute: 0, gender: 'female' };
  const tc = input => analyze(input, { asOf: AS_OF }).timeContext;
  ...
  assert.throws(() => analyze({ ...base, ziHourConvention: 'midnight' }, { asOf: AS_OF }), /ziHourConvention/);
  assert.throws(() => analyze({ ...base, timeAccuracy: 'roughly' }, { asOf: AS_OF }), /timeAccuracy/);
  assert.throws(() => analyze({ ...base, birthplace: { ...bp, timezone: '+09:00' } }, { asOf: AS_OF }), /timezone/);
  assert.throws(() => analyze({ ...base, useTrueSolarTime: 'yes' }, { asOf: AS_OF }), /useTrueSolarTime/);
