...
import { BirthData, createEngines } from '../src/index';
...
test('八字：五行與十神以十四次出現統計且包含日干', () => {
  ...
  assert.equal(Object.values(tenGods.value.counts as Record<string, number>).reduce((sum, count) => sum + count, 0), 14);
});
...
test('八字 B3：十神關係角色排除日主並產出固定顯隱統計', () => {
  ...
  assert.deepEqual(
    context.value.groups.map(({ group, context: label, count, presence }: any) => ({ group, context: label, count, presence })),
    [
  ...
  for (const group of context.value.groups) {
    ...
    assert.deepEqual(
      ...
      group.tenGods.filter((name: string) => group.breakdown[name] > 0),
    );
    assert.equal(
      Object.values(group.breakdown as Record<string, number>).reduce((sum, count) => sum + count, 0),
      group.count,
    );
...
test('八字 B3：十神群完全未出現時 presence 為無', () => {
  ...
  const context = result.byCategory('tenGodsContext')[0];
  const officers = context.value.groups.find((group: any) => group.group === '官殺');
...
test('八字：B2 asOf 格式與性別驗證錯誤', () => {
  ...
  assert.throws(() => {
    engine2!._compute({ year: 1991, month: 10, day: 5, hour: 14, minute: 0, gender: 'alien' } as any);
  }, /'male' or 'female'/);
});
