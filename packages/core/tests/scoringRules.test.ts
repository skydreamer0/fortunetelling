...
import { ScoringRules } from '../src/index';
import { BRIGHTNESS_WEIGHTS, brightnessScore } from '../src/engines/ZiweiEngine';
...
test('自訂規則需通過欄位驗證', () => {
  const rules = new ScoringRules();
  assert.throws(() => rules.addRule({ id: 'bad_rule' } as any), /missing required field/);
