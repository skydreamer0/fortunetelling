...
import { BirthData, ScoringRules } from '../src/index';
...
} from '../src/analysis/RadarBuilder';
import { AXIS_NOTES, getAxisNotes } from '../src/analysis/AxisNotes';
...
function assertRadarContract(radar: Record<string, any>) {
...
test('五行雷達：軸值總和 ≈ 100、單位 %、inputs 對應引擎實際次數', () => {
  ...
  const inputKeys: Record<string, string> = { 木: 'woodCount', 火: 'fireCount', 土: 'earthCount', 金: 'metalCount', 水: 'waterCount' };
  for (const axis of radar!.axes) {
...
test('數字頻次 bar：1991-10-05 → {1:3, 9:2, 5:1, 其餘 0}，單位次', () => {
  ...
  const expected: Record<string, number> = { 1: 3, 2: 0, 3: 0, 4: 0, 5: 1, 6: 0, 7: 0, 8: 0, 9: 2 };
  for (const axis of radar!.axes) {
...
test('D-010：兩軸同值輸入 → 同高（不做跨軸正規化）', () => {
  ...
  const radar = buildElementBalanceRadar(synthetic as any);
